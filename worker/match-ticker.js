...
// Raised from 0.88: fuzzy false positives burn trust on ticker, the field traders verify first.
...
export function matchConfidence(companyName, secTitle) {
...
function parseSecPayload(payload) {
...
async function fetchSecPayloadFromNetwork() {
  let lastError = null
...
async function loadSecTickerEntries(cache) {
  let cached = null
  try {
...
export async function fetchSecTickerEntries(cache) {
  if (!secFetchInFlight) {
...
export function findBestTickerMatch(companyName, entries, threshold = MATCH_CONFIDENCE_THRESHOLD) {
  const query = normalizeCompanyName(companyName)
...
export async function matchTickerForCompany(companyName, cache) {
...
export function createKvBindingStore(kv) {
  return {
    async get() {
      try {
        const payload = await kv.get(TICKER_CACHE_KEY, { type: 'json' })
        if (!payload || typeof payload !== 'object') return null
        if (!Array.isArray(payload.entries) || typeof payload.fetchedAt !== 'number') return null
        return payload
      } catch {
        return null
      }
...
export function createMemoryTickerCache() {
  let payload = null
  return {
    async get() {
      return payload
...
export function resolveTickerCache(env) {
  if (env?.TICKER_CACHE) {
    return createKvBindingStore(env.TICKER_CACHE)
  }
