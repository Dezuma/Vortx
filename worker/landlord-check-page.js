/**
 * SSR landing page for /landlord-check (public-record research, not tenant screening).
 */

import { LANDLORD_DISCLAIMER, LANDLORD_SOURCE_LABELS } from '../frontend/functions/lib/landlord-check.js'
...
.eyebrow{font:700 11px monospace;letter-spacing:.16em;text-transform:uppercase;color:#7c3aed;margin:0}
...
input:focus,select:focus{outline:2px solid #c4b5fd;outline-offset:1px}
.btn{display:inline-flex;align-items:center;justify-content:center;border:0;border-radius:14px;background:linear-gradient(135deg,#6d28d9,#8b5cf6);color:#fff;padding:15px 18px;font-size:16px;font-weight:800;cursor:pointer;width:100%}
...
.record .unlock{margin-top:10px;font:700 11px monospace;letter-spacing:.08em;text-transform:uppercase;color:#6d28d9}
...
.btn-secondary.primary{border-color:#6d28d9;background:#f5f3ff;color:#6d28d9}
...
const FAQ = [
  {
    q: 'How do I research public filings on a landlord name?',
    a: 'Enter the landlord, LLC, or property owner name and state. Vortx searches public liens, judgments, bankruptcy, receivership, and property-related distress filings we ingest. A match means a filing exists in our sources, not that you should refuse to rent. Full filing details unlock with a one-time $5 purchase.',
  },
  {
    q: 'What does a lien or judgment on a landlord mean?',
    a: 'A lien or judgment is a public filing that can signal payment stress or unresolved disputes. It is an administrative record, not proof of wrongdoing and not a tenant screen. Do not use this page to decide whether to rent to or from someone for an FCRA-covered purpose.',
  },
  {
    q: 'Is a clean result a guarantee the landlord is safe?',
    a: 'No. A clean result means we did not find matching filings in our current monitored sources for that name and state. Coverage varies by jurisdiction. Always read the lease, verify ownership, and follow local tenant-protection guidance. This is not a consumer report.',
  },
]
...
export function landlordCheckPage() {
  const site = 'https://vortxmkt.com'
  const title = 'Landlord Public-Records Search: Liens, Judgments and Distress Filings | Vortx'
  const description =
    'Research public liens, judgments, and distress filings tied to a landlord or property entity. Not a consumer report. Not for tenant screening or other eligibility decisions.'
  const canonical = `${site}/landlord-check`
  const sourcesHtml = LANDLORD_SOURCE_LABELS.map((line) => `<li>${esc(line)}</li>`).join('')
...
<html lang="en">
  ...
  <body>
    <main class="wrap">
      <div class="brand">
        <a href="/">Vortx</a>
        <span>Landlord Check · research only</span>
      </div>
      <p class="fcra-banner">${esc(FCRA_PRODUCT_BANNER)}</p>
      <section class="hero">
        <p class="eyebrow">Public-record research · not a consumer report</p>
        <h1>Research public filings tied to this landlord or property entity.</h1>
        <p class="lead">Enter a landlord, LLC, or owner name. Vortx searches public liens, judgments, bankruptcy, and property-related filings we ingest. Results can be incomplete or mismatched and are not a tenant-screening or eligibility decision.</p>
        <form id="landlord-form" class="grid-2" novalidate>
          <label>Landlord or property owner name
            <input id="landlord-name" name="name" type="text" autocomplete="organization" placeholder="Example: Harbor View Properties LLC" required />
          </label>
          <label>State
            <select id="landlord-state" name="state" aria-label="State">
              <option value="">Any state</option>
              ${stateOptions}
            </select>
          </label>
          <button class="btn" id="landlord-submit" type="submit">Check now</button>
        </form>
        <div class="trust">
          <p><strong>What we check:</strong> liens, judgments, bankruptcy, receivership, and property-related distress filings from sources we refresh daily.</p>
          ...
          <p>${esc(LANDLORD_DISCLAIMER)}</p>
        </div>
      </section>

      <section id="landlord-results" class="panel" aria-live="polite">
        <h2 id="landlord-result-headline">Results</h2>
        <p id="landlord-match-meta" class="legal hidden"></p>
        <p id="landlord-result-copy"></p>
        <div id="landlord-result-banner"></div>
        <div id="landlord-records"></div>
        <div id="landlord-unlock" class="cta-row hidden">
          <button type="button" class="btn-secondary primary" id="landlord-unlock-btn">$5 to see full details</button>
        </div>
        <aside id="landlord-clear-upsell" class="scout-upsell hidden">
          <p><strong>Clear in the free 180-day window is not the same as clear for a year.</strong></p>
          <p>Older liens, judgments, and distress filings often sit outside the free preview. Unlock a 12-month lookback for $5. This is still research only, not a consumer report.</p>
        </aside>
        <aside id="landlord-scout-upsell" class="scout-upsell hidden">
          <p><strong>${esc(SCOUT_SOFT_UPSELL.headline)}</strong></p>
          <p>${esc(SCOUT_SOFT_UPSELL.subcopy || '')}</p>
          <a id="landlord-subscribe-btn" href="${esc(SCOUT_SOFT_UPSELL.href)}">${esc(SCOUT_SOFT_UPSELL.cta)}</a>
        </aside>
        <div id="landlord-no-result-extra" class="hidden">
          <p class="legal">We check:</p><ul>${sourcesHtml}</ul><p class="legal">Absence of a record is not a guarantee. Always read the lease and verify ownership separately.</p>
        </div>
      </section>
      ...
      <p class="legal">${esc(LANDLORD_DISCLAIMER)} <a href="/legal">Legal</a> · <a href="/?view=sources">Data sources</a></p>
    </main>
    <script src="/analytics.js" defer></script>
    <script src="/landlord-check.js"></script>
  </body>
</html>`
