...
| **Funnel** | One primary path: capture attention → single next action (waitlist, newsletter, book a call, try the widget). |
...
**Cloudflare Pages** fits this repo: connect the Git repo, run **`npm run build`**, publish **`dist`**, set **`VITE_*`** env vars in the Pages project (never in git). Add **Workers** or **Pages Functions** later for thin BFF logic, geo hints, or rate limits—while keeping Supabase as the source of truth for data and RLS.
...
| Hosting | **Cloudflare Pages** | Global static + SPA fallback; Workers/Pages Functions when you need edge logic. |
