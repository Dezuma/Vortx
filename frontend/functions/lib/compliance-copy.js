...
export function containsForbiddenTerm(value) {
  return FORBIDDEN_RE.test(String(value || ''))
}

export function assertCompliantCopy(value, context = 'copy') {
  const text = String(value || '')
  if (containsForbiddenTerm(text)) {
    const error = new Error('forbidden_copy')
    error.status = 400
    error.body = {
      ok: false,
      error: 'forbidden_copy',
      message: `Generated ${context} contains non-compliant language.`,
    }
    throw error
  }
  return text
...
export function entityScanHeadline(entityName, count) {
...
export function noSignalsMessage(entityName) {
  const name = String(entityName || 'This entity').trim()
...
export function adjacentSignalsMessage(entityName, count, scopeLabel) {
  const name = String(entityName || 'This entity').trim()
  const n = Number(count) || 0
