...
-- Safe int parse for JSON text that may be "86", "86.0", or blank.
...
begin
  ...
  loop
    begin
      ...
      if event_uuid is not null and nullif(trim(event_payload->>'evidence_url'), '') is not null then
        ...
        where not exists (
          ...
          where ee.event_id = event_uuid
            and ee.source_url = event_payload->>'evidence_url'
        );
      end if;
